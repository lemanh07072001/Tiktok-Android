...
scr=sess.create_script(open("_slot16full.js").read()); scr.on("message",on); scr.load()
...
print("captured url=%s slot16_hex=%s"%(meta["url"],meta["slot16_hex"]),flush=True)
...
json.dump({"meta":meta,"closure":closure,"so_chunks":chunks},open("_slot16full_out.json","w"))
