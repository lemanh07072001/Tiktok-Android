...
Interceptor.attach(base.add(EXIT_PATH),{
  onEnter(){
    ...
    const x25=c.x25,x22=c.x22,x20=c.x20,x1=c.x1,x23=c.x23,x28=c.x28,x19=c.x19,x26=c.x26;
    let ctl={};
    ...
    if(!x22.isNull()&&x22.compare(0x10000)>0){
      cb.x22=x22.toString(16);cb.fnptr=ru64(x22);
      try{const f=x22.readU64();if(f.compare(0)!==0)cb.code=rp(ptr(f),32);}catch(e){}
    }
    send({t:'exit',n:hits,x25:x25.toString(16),x26:x26.toString(16),x22:x22.toString(16),x20:x20.toString(16),x1:x1.toString(16),x23:x23.toString(16),x28:x28.toString(16),x19:x19.toString(16),ctl:ctl,cb:cb,regfile_x1:rp(x1,256)});
  }
});
send({t:'info',msg:'PERSIST exit hook installed. Waiting for VM activity...'});
