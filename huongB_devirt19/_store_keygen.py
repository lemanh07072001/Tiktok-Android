...
PKG='com.zhiliaoapp.musically'; JS='_store_keygen.js'; OUT='cap.noindex/store_keygen.json'
...
def on_msg(m,d):
    if m.get('type')=='send':
        ...
        if tag=='GEN':
            print('  << GEN %s ret=%s retMem=%s'%(p['name'],(p.get('ret') or '')[-8:],(p.get('retMem') or '')[:48]))
            for i,mm in enumerate(p.get('memIn') or []):
                if mm: print('        in[%d]=%s'%(i,mm[:48]))
            for i,mm in enumerate(p.get('memOut') or []):
                if mm: print('       out[%d]=%s'%(i,mm[:48]))
        elif tag=='EINIT':
            print('  << EINIT kb=%s key=%s iv=%s'%(p.get('kb'),p.get('key'),p.get('iv')))
        elif tag in ('READY','WAIT_DLOPEN'): print('  <<',tag,p.get('base') or '')
...
g=[m for m in msgs if m.get('tag')=='GEN']; e=[m for m in msgs if m.get('tag')=='EINIT']
print('=== GEN events=%d  EINIT=%d ==='%(len(g),len(e)))
print('WROTE',OUT)
try: scr.unload()
except: pass
