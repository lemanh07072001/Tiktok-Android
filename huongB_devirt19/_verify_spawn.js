...
function doInstall(base){
  ...
  Interceptor.attach(base.add(0x15b594),{
    ...
    onLeave(){if(this.inp!==null){md5log.push({out:hx(this.out,16),inhex:this.inp,len:this.len});if(md5log.length>400)md5log.shift();}}
  });
  const chain={};
  Interceptor.attach(base.add(SM3),{onEnter(){
    ...
    if(slot!=='00000000000000000000000000000000'){
      let found=null;
      for(let k=md5log.length-1;k>=0;k--){if(md5log[k].out===slot){found=md5log[k];break;}}
      send({t:'V',slot16:slot,md5match:found,md5count:md5log.length});
    }
    delete chain[tid];
  }});
...
if(m)doInstall(m.base);
else{const dl=Module.findGlobalExportByName('android_dlopen_ext')||Module.findGlobalExportByName('dlopen');
  Interceptor.attach(dl,{onEnter(a){try{this.p=a[0].readCString();}catch(e){}},onLeave(){if(this.p&&this.p.indexOf(SO)>=0){const mm=Process.findModuleByName(SO);if(mm)doInstall(mm.base);}}});}
