...
function rdN(p,n){ try{ const u=new Uint8Array(p.readByteArray(n)); let s=''; for(let i=0;i<n;i++)s+=('0'+u[i].toString(16)).slice(-2); return s; }catch(e){return null;} }
function inSelf(p){ try{ return p.compare(lo)>=0 && p.compare(hi)<0; }catch(e){ return false; } }
const NAMES=[];for(let i=0;i<=28;i++)NAMES.push('x'+i);
function install(){
  ...
  Interceptor.attach(base.add(DISP),{ onEnter(a){
    ...
    let op=null; try{ const pcp=c.x23.readPointer(); op=pcp.readU32()&0x3f; }catch(e){}
    const R={};
    for(const nm of NAMES){ try{ const v=c[nm]; R[nm]=v.toString(); const mm=rdN(v,96); if(mm) R[nm+'m']=mm; }catch(e){} }
    ring[ri%RING]={s:seq++, op:op, R:R}; ri++;
  }});
  Interceptor.attach(base.add(SM3),{ onEnter(a){
    ...
    if(w1!==16) return;
    const V=rdN(c.x0,16); if(!V || V==='00000000000000000000000000000000') return;
    dumped=true; const P=c.x0.toString();
