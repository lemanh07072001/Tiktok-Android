...
time.sleep(2)
json.dump(out,open("_ctxstalker.json","w")); print("[SAVED-JSON] mem=%d trace=%d slot16=%s"%(len(out["mem"]),len(out["trace"]),(out.get("ctx") or {}).get("slot16")),flush=True)
try: sess.detach()
except: pass
