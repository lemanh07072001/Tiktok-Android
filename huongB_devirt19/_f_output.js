...
const CANDS={};[0x17c880,0x18f430,0x191f40,0x184780,0x190140].forEach(x=>CANDS[x]=1);
...
function install(){
  ...
  Interceptor.attach(base.add(VM),{
    ...
    onLeave(){
      if(!this.rec) return;
      const bufs=[];
      grab(this.x4,64,2,bufs);      // outbuf, 2-level deref
      grab(this.x1,0x60,2,bufs);    // inbuf object graph, 2-level
      roll.push({prog:this.prog,bufs:bufs});
      if(roll.length>400) roll.shift();
    }
  });
