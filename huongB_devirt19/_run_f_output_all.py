...
PID=int(sys.argv[1]); SECS=int(sys.argv[2]) if len(sys.argv)>2 else 75; CAP=int(sys.argv[3]) if len(sys.argv)>3 else 12
nz=[]
def on_msg(m,d):
    ...
    if t=="info": print("[*]",p["msg"],flush=True)
        ...
        nz.append(p); print("[NZ #%d] slot16=%s hits=%s qhead=%s"%(len(nz),p["slot16"],p["hits"],p["qhead"][:30]),flush=True)
dev=frida.get_device_manager().add_remote_device("127.0.0.1:47119")
sess=dev.attach(PID)
src="const FO_CAP=%d;\n"%CAP+open("_f_output_all.js",encoding="utf-8").read()
...
json.dump({"nz":nz},open("_f_output_out.json","w")); print("[DONE] nz=%d"%len(nz),flush=True)
