...
// Capture ALL md5 outputs (finalize 0x15b43c AND one-shot out) with their input
const md5log=[];  // {out, inhex, len}
Interceptor.attach(base.add(0x15b594),{
  ...
  onLeave(){if(this.inp!==null){md5log.push({out:hx(this.out,16),inhex:this.inp,len:this.len});if(md5log.length>300)md5log.shift();}}
});
// SM3 slot16
const chain={};
Interceptor.attach(base.add(SM3),{onEnter(){
  ...
  if(slot!=='00000000000000000000000000000000'){
    // search md5log for this slot as an OUTPUT
    let found=null;
...
send({t:'info',msg:'verify hook ready'});
