import frida,sys,time,json,os
PKG="com.zhiliaoapp.musically"
DUR=int(sys.argv[1]) if len(sys.argv)>1 else 80
OUT=os.path.join(os.path.dirname(__file__),"..","ground-truth","getseed_aead_ce0516.json")
JS=r"""
function asc(p,n){try{var b=new Uint8Array(p.readByteArray(Math.min(n,1600)));var s="";for(var i=0;i<b.length;i++){var c=b[i];s+=(c>=32&&c<127)?String.fromCharCode(c):".";}return s;}catch(e){return"";}}
function hx(p,n){try{var b=new Uint8Array(p.readByteArray(Math.min(n,1600)));var h="";for(var i=0;i<b.length;i++)h+=("0"+b[i].toString(16)).slice(-2);return h;}catch(e){return"";}}
function interesting(s){return s.indexOf("get_seed")>=0||s.indexOf("/ms/")>=0||s.indexOf("mssdk")>=0||s.indexOf("ms_query")>=0||s.indexOf("device_platform")>=0;}
var lc=Process.findModuleByName("libcrypto.so");
var SEAL=lc.findExportByName("EVP_AEAD_CTX_seal");
var OPEN=lc.findExportByName("EVP_AEAD_CTX_open");
var nSeal=0,nOpen=0;
Interceptor.attach(SEAL,{onEnter:function(a){try{
  nSeal++;var inp=a[6],inl=a[7].toInt32();if(inl<=0||inl>200000)return;
  var s=asc(inp,inl);
  if(interesting(s)){send({tag:"SEAL",dir:"OUT",len:inl,ascii:s,hex:hx(inp,inl)});}
}catch(e){}}});
Interceptor.attach(OPEN,{onEnter:function(a){this.out=a[1];this.olp=a[2];},onLeave:function(r){try{
  nOpen++;if(r.toInt32()!==1)return;var n=this.olp.readULong?this.olp.readULong():this.olp.readU32();
  n=parseInt(n);if(!n||n<=0||n>200000)return;
  var s=asc(this.out,n);
  if(interesting(s)){send({tag:"OPEN",dir:"IN",len:n,ascii:s,hex:hx(this.out,n)});}
}catch(e){}}});
send({tag:"info",msg:"hooked seal@"+SEAL+" open@"+OPEN});
setInterval(function(){send({tag:"stat",seal:nSeal,open:nOpen});},10000);
"""
ev=[]
def om(m,d):
    if m.get("type")=="send":
        p=m["payload"];t=p["tag"]
        if t=="info":print("[*]",p["msg"],flush=True)
        elif t=="stat":print(f"[stat] seal={p['seal']} open={p['open']}",flush=True)
        elif t in("SEAL","OPEN"):
            print(f"\n[+++ {t} {p['dir']}] len={p['len']}\n  ascii={p['ascii'][:200]}\n  hex={p['hex'][:120]}",flush=True)
            ev.append(p)
    elif m.get("type")=="error":print("[ERR]",m.get("description"),flush=True)
ADB=os.popen("cat /tmp/adbpath").read().strip()
os.environ["MSYS_NO_PATHCONV"]="1"
print("[*] full-clear trigger",flush=True)
os.system(f'"{ADB}" shell \'su -c "am force-stop {PKG}; rm -rf /data/data/{PKG}/files/.msdata/mssdk/ov; rm -f /data/data/{PKG}/files/keva/repo/mssdk/*"\' 2>&1')
dev=frida.get_usb_device(timeout=10)
print(f"[*] spawn {PKG} {DUR}s AEAD",flush=True)
pid=dev.spawn([PKG]);s=dev.attach(pid)
sc=s.create_script(JS);sc.on("message",om);sc.load();dev.resume(pid)
t0=time.time()
while time.time()-t0<DUR:time.sleep(0.5)
try:s.detach()
except:pass
json.dump(ev,open(OUT,"w"),indent=1)
print(f"\n=== {len([e for e in ev if e['tag']in('SEAL','OPEN')])} interesting AEAD events -> {OUT}",flush=True)
