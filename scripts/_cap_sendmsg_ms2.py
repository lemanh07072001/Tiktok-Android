...
var mbase=null,mend=null;
function refind(){var M=Process.findModuleByName("libmetasec_ov.so");if(M){mbase=M.base;mend=M.base.add(M.size);return true;}return false;}
var iv=setInterval(function(){if(refind()){clearInterval(iv);send({tag:"info",msg:"MS loaded @"+mbase});}},120);
...
function iovdump(mh){try{
  ...
  var name=mh.readPointer();var peer=sockaddr(name);var tot=0,first="";
...
Interceptor.attach(SM,{onEnter:function(a){nS++;if(!fromMS(this.returnAddress))return;msS++;var d=iovdump(a[1]);if(d)send({tag:"SEND",peer:d.peer,len:d.tot,first:d.first});}});
Interceptor.attach(RM,{onEnter:function(a){this.mh=a[1];this.ms=fromMS(this.returnAddress);},onLeave:function(r){nR++;if(!this.ms)return;var n=r.toInt32();if(n<=0)return;msR++;var d=iovdump(this.mh);if(d)send({tag:"RECV",peer:d.peer,len:n,first:d.first});}});
send({tag:"info",msg:"hooked sendmsg@"+SM+" recvmsg@"+RM});
...
def om(m,d):
    if m.get("type")=="send":
        ...
        elif t in("SEND","RECV"):print(f"[{t}] peer={p['peer']} len={p['len']} first={p['first'][:80]}",flush=True);ev.append(p)
    elif m.get("type")=="error":print("[ERR]",m.get("description"),flush=True)
dev=frida.get_usb_device(timeout=10)
print(f"[*] spawn {PKG} {DUR}s (deferred MS base)",flush=True)
...
for pr in sorted(set(e["peer"] for e in ev)):print("   peer:",pr,flush=True)
