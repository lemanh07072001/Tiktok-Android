...
## #24 message-copy plumbing (2026-09-04) — the remaining wall
- WHOOK on the #24 member slot (0xe4ffde10 = msg+0xe8): PC 0x3d1990 sets it = 0x12517680 (a real string ptr) during the message BUILD; but at serialize time (0x154f24) the member reads 0x12196e5a = the proto3 DEFAULT empty-string sentinel (shared rodata).
- ⇒ the BUILD message (0x3d1990, #24=0x12517680) and the SERIALIZE message (0x154f24, #24=default) are DIFFERENT instances (a message copy/move between build and serialize drops #24, or #24 is only in a source not copied).
- Filling the default sentinel (mode10) doesn't help (shared rodata + it's the "empty" marker). The f23-writer injection (mode7) sets the serialize member right before f24 and DOES emit the 44-byte base64-DUID, but crashes on buffer overflow (the ByteSizeLong pass sized the buffer with #24 as the empty default).
## FINAL (#24): encoding fully solved (C-string). Emitting a byte-correct #24 requires either (a) making the message-copy carry #24 (find the copy at 0x3d1990's downstream), or (b) injecting at the f23-writer in the SIZE pass too AND enlarging the buffer + AES length. Multiple message-plumbing layers remain. VALUE: zero demonstrated (no endpoint validates x-argus content). RECOMMENDATION: do not invest further in full-772 without first proving x-argus content matters somewhere (login/ec7 with credentials).
